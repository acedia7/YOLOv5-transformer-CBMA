...
from models.common import Conv, Bottleneck, SPP, DWConv, Focus, BottleneckCSP, Concat, NMS, autoShape, CBAM,BottleneckCSP_transformer
...
class Detect(nn.Module):
    ...
    '''
    在前向传播过程中，该类会遍历每个检测层，并执行以下步骤：
        将输入通过一个1x1的卷积层（self.m[i]），以准备生成预测。
        调整输出张量的形状，以便表示边界框和目标置信度的预测。
        如果不在训练模式下：
            计算边界框的坐标和目标置信度。
            使用网格和锚框信息调整边界框的预测。
            将预测结果添加到列表z中。
        如果在训练模式下，返回的是中间结果x；否则，返回的是拼接的预测结果（z）和中间输出（x）。
    '''


# 网络模型类
class Model(nn.Module):
    def __init__(self, cfg='yolov5s-voc.yaml', ch=3, nc=None):  # model, input channels, number of classes
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, Bottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, C3, BottleneckCSP_transformer,CBAM]:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov5s-voc.yaml', help='model.yaml')
